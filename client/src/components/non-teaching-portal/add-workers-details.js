...
class AddWorkersDetails extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            userID: '',
            staffDetails: [],
            values: [{ name: '', designation: '', qualification: '', contact: '', departmentName: '', email: '', emergencyContact: '', joiningDate: moment(new Date()).format('YYYY-MM-DD') }]
        };
     ...
     */
    resetForm() {
        this.setState({ values: [{ name: '', designation: '', qualification: '', contact: '', email: '', departmentName: '', emergencyContact: '', joiningDate: moment(new Date()).format('YYYY-MM-DD') }] })
    }
        ...
        if (this.state.values && this.state.values.length > 0) {
            this.state.values.forEach(element => {
                if (element.name && element.designation && element.qualification && element.joiningDate
                    ...
                    axios.post(`/xakal/nonteaching`, params)
                        .then(() => {
                            if (!isUpdated) {
                                ...
                                {this.state.values.map((el, i) =>
                                    <div className="card-body row" key={i}>
                                        ...
                                        <div className="col-lg-1 p-t-20">
                                            <div
                                                className={"mdl-textfield mdl-js-textfield mdl-textfield--floating-label getmdl-select getmdl-select__fix-height select-width "}>
                                                <input value={el.departmentName} autoComplete="off" onBlur={this.onFocusOut.bind(this)} onFocus={this.onDropDownFocus.bind(this)} className="mdl-textfield__input display-border" type="text" id="departmentName"
                                                    onChange={this.handleFormChange.bind(this, i)} name="departmentName" />
